#!/usr/bin/env python3
"""Convert MPC-005 and MPC-006 to MPCE-012 format."""
...
def extract_questions(html_content):
    """Extract questions from MPC-005/006 style HTML (div class="content")."""
    questions = []
    ...
    for details in details_matches:
        ...
        spans = re.findall(r'<span[^>]*>([^<]*)</span>', summary_text)
        question_title = ' '.join([s.strip() for s in spans if s.strip()])
        
        if not question_title:
            # Fallback: remove all tags
            question_title = re.sub(r'<[^>]+>', ' ', summary_text).strip()
        
        # Extract year from time tag if present
        time_match = re.search(r'<time[^>]*>([^<]*)</time>', summary_text)
        years = ""
        if time_match:
            years = time_match.group(1).strip()
        
        # Extract answer from div class="content"
        answer_match = re.search(r'<div[^>]*class="content"[^>]*>(.*?)<div[^>]*class="button-group"', details, re.DOTALL | re.IGNORECASE)
