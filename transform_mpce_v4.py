...
def transform_file(input_path, output_path):
    ...
    while i < len(lines):
        ...
        # RULE 6: Exam_usse - may span multiple lines (look ahead for closing quote)
        ...
        if exam_match:
            ...
            if text.endswith('"'):
                ...
                # Look for closing quote on next line(s)
                j = i + 1
                while j < len(lines) and not lines[j].strip().endswith('"'):
                    text += " " + lines[j].strip()
                    j += 1
                if j < len(lines):
                    final_part = lines[j].strip()
